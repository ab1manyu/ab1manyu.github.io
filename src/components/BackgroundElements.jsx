...
export default function BackgroundElements() {
  ...
  useEffect(() => {
    ...
    const handleRadarPing = () => {
      ...
      for (let i = 0; i < targetCount; i++) {
        ...
        const rotation = isLeft ? 45 + Math.random() * 45 : -45 - Math.random() * 45;
  ...
  return (
    <>
      <div id="radar-screen-sweep" className={`fixed top-0 left-0 w-[50px] h-[50px] rounded-full border-2 border-defense-accent pointer-events-none z-0 ${radarActive ? 'full-sweep-active' : 'opacity-0'}`}></div>

      {bogeys.map((bogey) => {
        return (
          <div key={bogey.id} className="fighter-jet-dynamic fixed z-0 flex flex-col items-center gap-1 pointer-events-none"
            style={{
              top: `${bogey.top}%`,
          ...
          >
            <Plane className="w-6 h-6 text-defense-accent" style={{ transform: `rotate(${bogey.rotation}deg)` }} />
            <span className="text-[8px] font-mono text-defense-accent bg-black/80 px-1 border border-defense-accent">BOGEY_0{bogey.index + 1}</span>
          </div>
        );
      })}
      
      {/* PAGE TRANSITION OVERLAY */}
      <div id="transition-overlay" className={fadedOut ? "faded-out" : ""}></div>
    </>
  );
}
