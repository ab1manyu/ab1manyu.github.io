...
export default function Navbar() {
  ...
  const isGallery =
    location.pathname === "/gallery" || location.pathname === "/gallery.html";
  ...
  const pokedexAction = (action, value) => {
    window.dispatchEvent(
      new CustomEvent("pokedex-action", { detail: { action, value } })
    );
  };
      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="relative flex items-center justify-center w-10 h-10">
            <div
              id="radar-pulse"
              className="absolute w-full h-full border border-defense-accent rounded-full opacity-0 scale-0 pointer-events-none"
            ></div>
            ...
            {isGallery ? (
              ...
              >
                <Target
                  className={`w-6 h-6 transition-all duration-300 ${bullseyeActive ? "text-emerald-400 scale-125 rotate-45" : "text-defense-accent"}`}
                />
              </button>
            ) : (
              <button
          ...
          <div className="flex flex-col leading-none min-w-[110px]">
            ...
            {isGallery ? (
              <span
                className={`font-mono text-[10px] transition-colors duration-200 whitespace-nowrap ${shutterStatus === "SYS_ONLINE" ? "text-gray-500" : "text-defense-accent"}`}
              >
                {shutterStatus}
              </span>
            ) : isPokedex ? (
              <span
                className={`font-mono text-[10px] transition-colors duration-200 whitespace-nowrap ${pokedexStatus === "SYS_ONLINE" ? "text-gray-500" : "text-defense-accent"}`}
              >
                {pokedexStatus}
              </span>
            ) : isKai ? (
              <span
                className={`font-mono text-[10px] transition-colors duration-200 whitespace-nowrap ${catStatus === "SYS_ONLINE" ? "text-gray-500" : "text-defense-accent"}`}
              >
                {catStatus}
              </span>
            ) : isScore ? (
              <span
                className={`font-mono text-[10px] whitespace-nowrap transition-colors duration-200 ${bullseyeActive ? "text-emerald-400 font-bold" : "text-gray-500"}`}
              >
              ...
              <span
                id="radar-status"
                className={`font-mono text-[10px] whitespace-nowrap ${radarStatus === "SYS_ONLINE" ? "text-gray-500" : "text-defense-accent"}`}
              >
                {radarStatus}
              </span>
            )}
        ...
        <div className="flex-1 flex justify-center">
          {isPokedex ? (
            ...
            <div className="hidden md:flex gap-8 text-sm font-mono tracking-wider text-defense-muted">
              <Link
                to="/#about"
                onClick={(e) => handleScrollToHash(e, "#about")}
                className="nav-item hover:text-white transition-colors py-1"
              >
                01_INTEL
              </Link>
              <Link
                to="/#skills"
                onClick={(e) => handleScrollToHash(e, "#skills")}
                className="nav-item hover:text-white transition-colors py-1"
              >
                02_OPS
              </Link>
              <Link
                to="/#contact"
                onClick={(e) => handleScrollToHash(e, "#contact")}
                className="nav-item hover:text-white transition-colors py-1"
              >
                03_LINK
              </Link>
