...
export default function PokemonDetail({ pokemon, onClose }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const flavorEntry = jsonSpecies.flavor_text_entries.find(
        (e) => e.language.name === "en"
      );
      const description = flavorEntry
        ? flavorEntry.flavor_text.replace(/\f/g, " ")
        : "No description available.";
      const growthRate = jsonSpecies.growth_rate
        ? jsonSpecies.growth_rate.name
        : "Unknown";

      setData({ ...jsonPokemon, description, growthRate });
    } catch (err) {
      setError("Could not load data. Check your connection.", err);
    } finally {
      setLoading(false);
    }
  }, [pokemon.id]);
            ...
            <div className={styles.section}>
              ...
              <div className={styles.stats}>
                {data.stats.map((s) => {
                  ...
                  const pct = Math.min(
                    100,
                    Math.round((val / (STAT_MAX[name] || 200)) * 100)
                  );
                  const barColor =
                    pct < 35 ? "#e53935" : pct < 65 ? "#f9c74f" : "#43d47d";
                  return (
            ...
            <div className={styles.section}>
              ...
              <div className={styles.infoGrid}>
                ...
                <div className={styles.infoItem}>
                  ...
                  <span className={styles.infoVal}>
                    {capitalize(data.growthRate.replace("-", " "))}
                  </span>
                </div>
                <div className={styles.infoItem}>
