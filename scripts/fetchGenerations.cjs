...
async function fetchGeneration(genName, genId) {
  ...
  console.log(
    `Found ${speciesList.length} species for ${genName}. Fetching types...`
  );
  ...
  for (let i = 0; i < speciesList.length; i += BATCH_SIZE) {
    ...
    const batchResults = await Promise.all(
      batch.map(async (species) => {
        ...
        try {
          const pkmnData = await fetchWithRetry(
          ...
          console.error(
            `Error fetching ${species.name} (${species.id}):`,
            e.message
          );
          return {
            id: species.id,
    ...
    process.stdout.write(
      `\rProgress: ${pokemonList.length}/${speciesList.length}`
    );
  }
